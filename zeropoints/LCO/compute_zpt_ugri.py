...
sm['u'],sm['g'],sm['r'],sm['i'] = USNO2LCO(sloan_mags['bd+174708']['u_40'], 
      sloan_mags['bd+174708']['g_40'], sloan_mags['bd+174708']['r_40'],
      sloan_mags['bd+174708']['i_40'])
print "Assuming standard and natural magnitdes:"
for f in fs:
   print f,"standard:  ",sloan_mags['bd+174708'][f+'_40'], ', natural:',sm[f]

# Do this three times, just to see who different SED's change the answer
print "zero-points with the original BD+174708:"
for f in fs:
   zpt = fset[f].compute_zpt(sloan_standards['bd+174708'], sm[f], zeropad=1)
   print f, zpt

print "zero-points with Mark's modified BD+174708:"
for f in fs:
   zpt = fset[f].compute_zpt(sloan_standards['bd+174708adj'], sm[f], 
         zeropad=1)
   print f, zpt

print "zero-points with Bohlin & Gilliland (2004) SED:"
for f in fs:
   zpt = fset[f].compute_zpt(sloan_standards['bd+17_Bohlin'], sm[f], 
         zeropad=1)
   print f, zpt
