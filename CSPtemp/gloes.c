 ...
 * an adaptive weight algorithm and work in 2D by Chris Burns (OCIW).  Ported
 * to C by Chris Burns*/
...
int fitpoly(gsl_vector *x, gsl_vector *y, gsl_vector *w, double x0, int n, int k, 
      gsl_vector *c, gsl_vector *cerror, double *chisq) {
   /* Fit a 1D polynomial of degree k to the data (x,y,weight) (length n)
    * around the point x0.  The coefficients are returned as (k+1)^2 length double
